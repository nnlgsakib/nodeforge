...
- [ ] Task 1 (AC: 1, 2) — Enhance CanvasControls with heat visualization and keyboard navigation
  - [ ] Subtask 1.1: Add execution heat effect to MiniMap (nodes glow based on recent activity, e.g., last 5 minutes)
  - [ ] Subtask 1.2: Implement Vim key navigation (h=left, j=down, k=up, l=right) via keydown event listener on canvas
  - [ ] Subtask 1.3: Implement Emacs key navigation (Ctrl-f=forward/right, Ctrl-b=back/left, Ctrl-n=next/down, Ctrl-p=previous/up) for canvas panning
  - [ ] Subtask 1.4: Ensure zoom/pan controls (from @xyflow/react Controls) are styled and functional with Tailwind + CSS variables

- [ ] Task 2 (AC: 3) — Implement SessionExplorer session management with search/filter
  - [ ] Subtask 2.1: Fetch sessions from GET /api/v1/sessions (REST API endpoint)
  - [ ] Subtask 2.2: Display session list with status (running/complete/failed), creation date, project name
  - [ ] Subtask 2.3: Add resume button (calls POST /api/v1/sessions/:id/resume or CLI nforge session resume)
  - [ ] Subtask 2.4: Add fork button (clones session via API or WebSocket message)
  - [ ] Subtask 2.5: Add export button (triggers nforge session export <id> or API download)
  - [ ] Subtask 2.6: Implement search input (filter by project name, case-insensitive)
  - [ ] Subtask 2.7: Add status filter dropdown (All/Running/Complete/Failed)
  - [ ] Subtask 2.8: Add date filter (All/Today/Week/Month)

- [ ] Task 3 (AC: 4) — Create NodeConfig dialog (Radix Dialog) with real-time validation
  - [ ] Subtask 3.1: Create NodeConfig.tsx component using Radix Dialog primitive
  - [ ] Subtask 3.2: Add timeout input (number, min=1, max=300, real-time validation: "Timeout must be 1-300 seconds")
  - [ ] Subtask 3.3: Add retry count input (number, min=0, max=10, real-time validation: "Retry count must be 0-10")
  - [ ] Subtask 3.4: Add token budget slider/input (number, min=100, max=100000, real-time validation: "Budget must be 100-100000 tokens")
  - [ ] Subtask 3.5: Wire dialog open to double-click node or Enter key on selected node
  - [ ] Subtask 3.6: Save button applies config to node data via WebSocket message (type: "node_update") or API call
...
- `frontend/src/components/canvas/CanvasControls.tsx` (UPDATE)
- `frontend/src/components/panels/SessionExplorer.tsx` (UPDATE)
- `frontend/src/components/panels/node-config.tsx` (NEW)
- `frontend/src/components/panels/node-config.test.tsx` (NEW, co-located)
- `frontend/src/components/canvas/CanvasControls.test.tsx` (NEW, co-located)
- `frontend/src/components/panels/SessionExplorer.test.tsx` (NEW, co-located)
- `frontend/src/hooks/useWebSocket.ts` (MAYBE UPDATE)
- `frontend/src/App.tsx` (MAYBE UPDATE)
