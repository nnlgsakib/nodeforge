...
- [ ] Task 1: Implement Knowledge Graph Core (AC: 1)
  - [ ] Subtask 1.1: Create `internal/context/graph.go` with `KnowledgeGraph` struct, BadgerDB integration (dgraph-io/badger/v4)
  - [ ] Subtask 1.2: Implement `AddNodeOutput(nodeID, output string) error` to store node outputs as graph nodes with edge relationships
  - [ ] Subtask 1.3: Implement `BuildContext(nodeID string, maxTokens int) (string, error)` to assemble context from graph in <100ms (NFR-04)
  - [ ] Subtask 1.4: Implement `GetDownstreamContext(nodeID string) []ContextNode` to retrieve upstream/downstream context for memory reuse (FR18)
  - [ ] Subtask 1.5: Write unit tests for graph operations (add, query, edge traversal, context assembly timing)

- [ ] Task 2: Implement Node Memory Reuse (AC: 2)
  - [ ] Subtask 2.1: Create `internal/context/memory.go` with `NodeMemory` struct
  - [ ] Subtask 2.2: Implement `StoreMemory(nodeID, key, value string) error` to persist node outputs for downstream use
  - [ ] Subtask 2.3: Implement `GetMemory(nodeID, key string) (string, bool)` to retrieve upstream memory for current node
  - [ ] Subtask 2.4: Implement `InjectMemoryIntoPrompt(prompt string, nodeID string) string` to add memory context to LLM prompts
  - [ ] Subtask 2.5: Write unit tests for memory storage/retrieval and prompt injection

- [ ] Task 3: Implement Auto-Spec Generation (AC: 3)
  - [ ] Subtask 3.1: Create `internal/context/spec.go` with `SpecGenerator` struct
  - [ ] Subtask 3.2: Implement `GenerateSpec(nodeID string, output string) (string, error)` to auto-generate specs from node outputs
  - [ ] Subtask 3.3: Implement `AddSystemReferences(nodeID string, refs []string) error` to add system refs to graph (FR19)
  - [ ] Subtask 3.4: Implement `SpecToGraph(spec string) ([]Node, []Edge, error)` to convert generated specs into executable graph nodes
  - [ ] Subtask 3.5: Write unit tests for spec generation and graph conversion

- [ ] Task 4: Implement Context Overflow Handling (AC: 4)
  - [ ] Subtask 4.1: Create `internal/context/splitter.go` with `GraphSplitter` struct
  - [ ] Subtask 4.2: Implement `SplitGraphIfNeeded(nodeID string, maxTokens int) ([]SubGraph, error)` to auto-split graphs on overflow (FR20)
  - [ ] Subtask 4.3: Implement `SubGraph` struct with Nodes, Edges, EstimatedTokens fields
  - [ ] Subtask 4.4: Implement `EstimateSubGraphTokens(sg SubGraph) int` using ~4 chars/token heuristic (<10ms estimation)
  - [ ] Subtask 4.5: Write unit tests for graph splitting logic and token estimation

- [ ] Task 5: Define LLM Integration Points (AC: 1, 2, 3, 4)
  - [ ] Subtask 5.1: Create `internal/context/assembler.go` with `ContextAssembler` struct and `AssembleContext(nodeID string, maxTokens int) (string, error)` (called by LLM provider before invocation)
  - [ ] Subtask 5.2: Document integration interface: `ContextAssembler.AssembleContext()` to be called from `internal/llm/provider.go` (story 2.2) before LLM calls
  - [ ] Subtask 5.3: Implement `InjectContextIntoPrompt(prompt, context string) string` for use by story 2.4's `PromptOptimizer`
  - [ ] Subtask 5.4: Add performance guard: context assembly MUST complete in <100ms (NFR-04), fallback to naive prompt on timeout
  - [ ] Subtask 5.5: Write integration tests with mock LLM provider (from story 2.2)
